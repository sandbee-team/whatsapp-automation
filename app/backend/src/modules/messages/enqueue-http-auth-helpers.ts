...
import { STRONG_PASSWORD, uniqueEmail, uniqueIp } from './enqueue-test-support.js';

/**
 * enqueue-http-auth-helpers.ts (P11 Unit U3) - the signup/verify/login/
 * MFA/onboarding-walk HTTP fixture helpers, split out of
 * enqueue-test-support.ts for max-lines discipline (the
 * `instances-http-auth-helpers.ts` idiom). NOT itself a test file (no
 * `.test.ts` suffix).
 */
...
async function signupClientViaHttp(app: FastifyInstance, label: string): Promise<SignedUpClient> {
  ...
  const response = await app.inject({
    ...
    payload: {
      fullName: `Messages Test ${label}`,
      email,
      phoneE164: '+919876543210',
      companyName: `Messages Test Co ${label}`,
